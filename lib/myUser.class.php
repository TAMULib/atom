...
/**
   * Return an external auth provider config value from app.yml, with a default.
   *
   * Supports Symfony 1 app.yml style:
   *   all:
   *     oidc:
   *       auto_create_atom_user: true
   * ...which becomes sfConfig key: app_oidc_auto_create_atom_user
   *
   * We check OIDC first (since the template guard checks arOidcPlugin),
   * then CAS as a fallback.
   */
  public function getProviderConfigValue($key, $default = null)
  {
    // Common Symfony 1 naming for nested app.yml keys
    $oidcKey = 'app_oidc_' . $key;
    $casKey  = 'app_cas_' . $key;

    // Prefer OIDC if present
    $val = sfConfig::get($oidcKey, null);
    if (null !== $val)
    {
      return $val;
    }

    // Fallback to CAS if present
    $val = sfConfig::get($casKey, null);
    if (null !== $val)
    {
      return $val;
    }
  ...
  /**
   * Convenience logout method used by some templates/plugins.
   * Symfony 1 logout is typically handled by clearing credentials + attributes.
   */
	public function logout()
	{
	  ...
	  // Regenerate PHP session ID (safe, doesn’t rely on Symfony storage)
